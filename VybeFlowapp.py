## All model definitions start here, after imports and app/db initialization
## ...existing code...
...
class LivePoll(db.Model):
    ...
    options = db.Column(db.Text, nullable=False)  # JSON list of options
...
class User(db.Model):
    ...
    cover_photo = db.Column(db.String(120), default='default_cover.jpg')
    ...
    custom_background = db.Column(db.String(255), nullable=True)
    ...
    custom_theme_video = db.Column(db.String(120), nullable=True)
    profile_anthem_url = db.Column(db.String(255), nullable=True)
    profile_anthem_genre = db.Column(db.String(50), nullable=True, default='hip_hop')
    custom_layout = db.Column(db.Text, nullable=True)
...
class Post(db.Model):
    ...
    content_tag = db.Column(db.String(32), nullable=True)
...
class Message(db.Model):
    ...
    voice_filename = db.Column(db.String(120))
    timestamp = db.Column(db.DateTime, default=datetime.utcnow)
    is_read = db.Column(db.Boolean, default=False)
...
class Group(db.Model):
    ...
    description = db.Column(db.String(255))
    theme = db.Column(db.String(50), default='light')
    is_art_nudity = db.Column(db.Boolean, default=False)
...
class GroupChatMessage(db.Model):
    ...
    text = db.Column(db.String(1000))
    timestamp = db.Column(db.DateTime, default=datetime.utcnow)
    reply_to_id = db.Column(db.Integer, db.ForeignKey('group_chat_message.id'), nullable=True)
    is_announcement = db.Column(db.Boolean, default=False)
    is_pinned = db.Column(db.Boolean, default=False)
## Removed duplicate CrewMember model definition

## Removed duplicate Battle model definition

## Removed duplicate BattleVote model definition
...
def messages_with_username(username):
    if 'user_id' not in session:
        return redirect(url_for('login'))
    recipient = User.query.filter_by(username=username).first_or_404()
    if request.method == 'POST':
...
# <!-- Add this just before </body> in your base template (e.g., base.html or register.html) --> this just before </body> in your base template (e.g., base.html or register.html) -->
# <script>
# if ('serviceWorker' in navigator) {
#     window.addEventListener('load', function() {
#         navigator.serviceWorker.register('/static/service-worker.js')/static/service-worker.js')
...
# If you need to use Firebase Messaging, place this code in your Flutter/Dart project, not in your Python backend.# If you need to use Firebase Messaging, place this code in your Flutter/Dart project, not in your Python backend.

# (Removed invalid JavaScript/React and CSS code. If needed, place this code in the appropriate frontend files.)# (Removed invalid JavaScript/React and CSS code. If needed, place this code in the appropriate frontend files.)
...
## Removed duplicate '/send_message' route and function
